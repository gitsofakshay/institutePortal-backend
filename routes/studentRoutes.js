...
router.post('/addstudent', authMiddleware, [
...
router.put('/updatestudent/:id', authMiddleware, [
    body('name').optional().isLength({ min: 3 }).withMessage('Name must be at least 3 characters'),
    body('dob').optional().isLength({ min: 10, max: 10 }).withMessage('Please enter correct date'),
    body('email').optional().isEmail().withMessage('Please enter a valid email'),
    body('phone').optional().isLength({ min: 10, max: 10 }).withMessage('Number must be 10 digits'),
    body('course').optional().isLength({ min: 2 }).withMessage('Course must be at least 2 characters'),
    body('gender').optional().isLength({ min: 4 }).withMessage('Please provide correct gender'),
    body('address').optional().isLength({ min: 5 }).withMessage('Address must be at least 5 characters'),
    body('fees.total').optional().isNumeric().withMessage('Total fee must be a number')
], async (req, res) => {
    let success = false;
    ...
    try {
        ...
        const {
            name, dob, email, phone, course, gender, address, enrolled,
            fees
        } = req.body;
        ...
        if (fees && fees.total !== undefined) {
            ...
            const incomingTotal = Number(fees.total); // Convert to number safely
            const updatedTotal = student.fees.total + incomingTotal;
        
            newStudent.fees.total = updatedTotal;
            newStudent.fees.due = updatedTotal - (student.fees.paid || 0);
        }
        ...
        success = true;
        res.status(200).json({ success, msg: 'Student data has been updated', stu: updatedStudent });
    } catch (error) {
        console.error(error.message);
...
router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        res.json({ message: 'Manual payment recorded successfully', fees: student.fees });
      } catch (error) {
        console.error(error.message);
